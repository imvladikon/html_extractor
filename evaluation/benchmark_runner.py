...
class BenchmarkRunner:
    ...
    def __init__(self, table_folder, metadata_folder):
        self.report = None
    ...
    @classmethod
    def from_path(cls, table_folder, metadata_folder):
        return cls(table_folder, metadata_folder)
    ...
    def run(self, predictor: Union[str, AbstractTransformer]):
        ...
        if isinstance(predictor, str):
            predictor = TransformerFactory.create(predictor)

        report_data = []
        html_files = list(Path(self.table_folder).glob('*.html'))
        for file in tqdm(html_files, desc=f"Benchmarking {predictor}", total=len(html_files)):
            table_id = file.stem
                ...
                for key in keys:
                    if key in predictions:
                        if isinstance(predictions[key], dict):
                            _calc(predictions[key], ground_truth[key], predictions[key].keys())
                        else:
